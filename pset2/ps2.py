...
class RectangularRoom(object):
    ...
    def getRandomPosition(self):
        ...
        randomPos = Position(randX, randY)
        return randomPos
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        roomX = int(math.floor(pos.getX()))
        roomY = int(math.floor(pos.getY()))
        return (roomX, roomY) in self.tiles
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        nextPosition = currentPosition.getNewPosition(currentDirection, self.speed)

        if robotRoom.isPositionInRoom(nextPosition):
            self.setRobotPosition(nextPosition)
            robotRoom.cleanTileAtPosition(nextPosition)
        else:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    while current_trial <= num_trials:
        ...
        for i in range(num_robots):
            robots_in_room.insert(i, robot_type(room, speed))
        
        time_steps = 0
        while (room.getNumCleanedTiles() / float(room.getNumTiles())) < min_coverage:
            #anim.update(room, robots_in_room)
            for robot in robots_in_room:
    ...
    mean = sum(trials) / float(num_trials)
    return mean
